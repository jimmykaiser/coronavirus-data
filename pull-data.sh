...
# Get data from today
TODAY=$(git log upstream/master --format=%ad --date=short -n 1 $today_hash | tr ' ' '-')
git cat-file -p $today_hash:data-by-modzcta.csv > "data-by-modzcta-$TODAY.csv"
# Get data from seven days prior
SEVEN_DAYS=$(git log upstream/master --format=%ad --date=short -n 1 $week_ago_hash | tr ' ' '-')
git cat-file -p $week_ago_hash:data-by-modzcta.csv > "data-by-modzcta-$SEVEN_DAYS.csv"
# Get data from two weeks prior
TWO_WEEKS=$(git log upstream/master --format=%ad --date=short -n 1 $two_week_hash | tr ' ' '-')
